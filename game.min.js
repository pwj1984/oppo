...
t;a[12]=d;a[13]=m;a[14]=s;return this};a.kmMat4Multiply=function(a,b,c){var d=a.mat,k=b.mat,m=c.mat;c=k[0];b=k[1];var n=k[2],p=k[3],s=k[4],r=k[5],u=k[6],t=k[7],v=k[8],w=k[9],z=k[10],x=k[11],y=k[12],A=k[13],B=k[14],k=k[15],C=m[0],D=m[1],E=m[2],F=m[3],G=m[4],H=m[5],I=m[6],J=m[7],K=m[8],L=m[9],M=m[10],N=m[11],O=m[12],P=m[13],Q=m[14],m=m[15];d[0]=C*c+D*s+E*v+F*y;d[1]=C*b+D*r+E*w+F*A;d[2]=C*n+D*u+E*z+F*B;d[3]=C*p+D*t+E*x+F*k;d[4]=G*c+H*s+I*v+J*y;d[5]=G*b+H*r+I*w+J*A;d[6]=G*n+H*u+I*z+J*B;d[7]=G*p+H*t+I*
x+J*k;d[8]=K*c+L*s+M*v+N*y;d[9]=K*b+L*r+M*w+N*A;d[10]=K*n+L*u+M*z+N*B;d[11]=K*p+L*t+M*x+N*k;d[12]=O*c+P*s+Q*v+m*y;d[13]=O*b+P*r+Q*w+m*A;d[14]=O*n+P*u+Q*z+m*B;d[15]=O*p+P*t+Q*x+m*k;return a};b.multiply=function(a){var b=this.mat,c=a.mat;a=b[0];var d=b[1],k=b[2],m=b[3],n=b[4],p=b[5],s=b[6],r=b[7],u=b[8],t=b[9],v=b[10],w=b[11],z=b[12],x=b[13],y=b[14],A=b[15],B=c[0],C=c[1],D=c[2],E=c[3],F=c[4],G=c[5],H=c[6],I=c[7],J=c[8],K=c[9],L=c[10],M=c[11],N=c[12],O=c[13],P=c[14],c=c[15];b[0]=B*a+C*n+D*u+E*z;b[1]=
B*d+C*p+D*t+E*x;b[2]=B*k+C*s+D*v+E*y;b[3]=B*m+C*r+D*w+E*A;b[4]=F*a+G*n+H*u+I*z;b[5]=F*d+G*p+H*t+I*x;b[6]=F*k+G*s+H*v+I*y;b[7]=F*m+G*r+H*w+I*A;b[8]=J*a+K*n+L*u+M*z;b[9]=J*d+K*p+L*t+M*x;b[10]=J*k+K*s+L*v+M*y;b[11]=J*m+K*r+L*w+M*A;b[12]=N*a+O*n+P*u+c*z;b[13]=N*d+O*p+P*t+c*x;b[14]=N*k+O*s+P*v+c*y;b[15]=N*m+O*r+P*w+c*A;return this};a.getMat4MultiplyValue=function(a,b){var c=a.mat,d=b.mat,k=new Float32Array(16);k[0]=c[0]*d[0]+c[4]*d[1]+c[8]*d[2]+c[12]*d[3];k[1]=c[1]*d[0]+c[5]*d[1]+c[9]*d[2]+c[13]*d[3];
...
n={r:n.r,g:n.g,b:n.b,a:a._displayedOpacity},p=a._itemWidth,s=0;s<d;s++){var r=c.charCodeAt(s)-a._mapStartChar.charCodeAt(0),u=r%a._itemsPerRow,t=0|r/a._itemsPerRow,v;cc.FIX_ARTIFACTS_BY_STRECHING_TEXEL?(u=(2*u*h+1)/(2*g),r=u+(2*h-2)/(2*g),t=(2*t*k+1)/(2*f),v=t+(2*k-2)/(2*f)):(u=u*h/g,r=u+h/g,t=t*k/f,v=t+k/f);var w=m[s],z=w.tl,x=w.tr,y=w.bl,w=w.br;z.texCoords.u=u;z.texCoords.v=t;x.texCoords.u=r;x.texCoords.v=t;y.texCoords.u=u;y.texCoords.v=v;w.texCoords.u=r;w.texCoords.v=v;y.vertices.x=s*p;y.vertices.y=
0;y.vertices.z=0;w.vertices.x=s*p+p;w.vertices.y=0;w.vertices.z=0;z.vertices.x=s*p;z.vertices.y=a._itemHeight;z.vertices.z=0;x.vertices.x=s*p+p;x.vertices.y=a._itemHeight;x.vertices.z=0;z.colors=n;x.colors=n;y.colors=n;w.colors=n}0<d&&(e.dirty=!0,a=e.totalQuads,d>a&&e.increaseTotalQuadsWith(d-a))};a.setString=function(a){a=a.length;a>this._textureAtlas.totalQuads&&this._textureAtlas.resizeCapacity(a)};a._addChild=function(){}})();cc.LABEL_AUTOMATIC_WIDTH=-1;
...
c,!0);u.responseType="arraybuffer";u.onload=function(){f.decodeAudioData(u.response,function(a){g.setBuffer(a);k(null,g)},function(){h.loadAudioFromExtList(c,d,g,k)})};u.send()}else{var t=document.createElement("audio"),v=!1,w=!1,z=setTimeout(function(){0===t.readyState?A():(w=!0,t.pause(),document.body.removeChild(t),k("audio load timeout : "+c,g))},1E4),x=function(){if(!v){t.pause();try{t.currentTime=0,t.volume=1}catch(a){}document.body.removeChild(t);g.setElement(t);t.removeEventListener("canplaythrough",
...
_onTouchBegan:function(a,b){console.log("_onTouchBegan");var c=Math.floor((a.getLocation().x-this.mapPanel.x)/Constant.CANDY_WIDTH),d=Math.floor((a.getLocation().y-this.mapPanel.y)/Constant.CANDY_WIDTH);this._popCandy(c,d);return!0},_onMouseDown:function(a){console.log("_onMouseDown");var b=Math.floor((a.getLocationX()-this.mapPanel.x)/Constant.CANDY_WIDTH);a=Math.floor((a.getLocationY()-this.mapPanel.y)/Constant.CANDY_WIDTH);this._popCandy(b,a)},_popCandy:function(a,b){if(!this.moving){for(var c=
